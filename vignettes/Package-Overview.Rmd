...
The aim of this package is to give suggestions for packages to be added to Task Views. There is a main function `CTVsuggest()` that outputs these suggestions.
...
### CTVsuggest() Example
Then to get the top 5 suggested packages for the **NaturalLanguageProcessing** Task View 
...
At the moment I have a script that I schedule that extracts the most up to date data, trains the model and then pushes the predicted classification probabilities for each package to a gitHub repo. In the latter sections I explain how the model is constructed.
...
The model aims to give a Task View suggestion after being giving some set of features of an unassigned package. There are quite a few packages that are assigned to multiple R packages, but I have looked at just classifying packages to a single Task View. Therefore, I have set this up as a multi-class classification problem. 
...
model also has the possibility of assigning a package to no Task View. Therefore, the number of possible labels in this classification problem is the number of Task Views + 1.
...
- Package and Task View text data
- Package dependencies
- Author collaborations

With each of these three types of data I have creates n + 1 features where n is the number of Task Views.
...
For each package I look at the immediate hard dependencies of packages, and then calculate the proportion
of the assignment of these packages to Task Views and no assignment.
